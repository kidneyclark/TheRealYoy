...
public partial class MapManager : Node3D
{
	...
	public Vector3 WorldPosFromTilePos(Vector2I tilePos, float hexHeight)
	{
		...
		float width = 2f * (size + 0.01f);
		float height = Mathf.Sqrt(3f) * (size + 0.01f);
	...
	public void GenerateMap()
	{
		...
		for (int x = 0; x < MapGenData.GridSize.X; x++)
		{
			for (int y = 0; y < MapGenData.GridSize.Y; y++)
			{
				...
				{
					tile = MapGenData.SandTilePrefab.Instantiate<Node3D>();
					tile.AddToGroup("Ground");
				}
				else if (elevation < 4.3f)
				{
				...
				{
					tile = MapGenData.SnowTilePrefab.Instantiate<Node3D>();
					tile.AddToGroup("Ground");
				}
